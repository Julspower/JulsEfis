...
bool EM7180_Master::begin(uint8_t bus)
{
    ...
    //Set passthru mode for warmstart
    _em7180.setRunEnable();
    _em7180.setPassThroughMode();
    QThread::usleep(200);
    char data[140];

    //Lecture de la page 255
    _em7180.M24512DFMreadBytes(0x50, 0x7F, 0x80, 12, &data[128]);
    QThread::usleep(100);
    //Lecture de la page 254
    _em7180.M24512DFMreadBytes(0x50, 0x7F, 0x00, 128, data);
    for (i = 0; i < 35; i++) // 35 parameters
    {
      for (j= 0; j < 4; j++)
      {
        Sen_param[i][j] = data[(i * 4 + j)];
      }
    }
    ...
    _em7180.setRunEnable();// set SENtral in initialized state to configure registers

    //Send readed parameter to chip
    _em7180.setCalibParameter(Sen_param);
    ...
    //Beginning parameter adjustements
...
{
    uint16_t c = _em7180.getRamVersion();
