/* 
   ...
   EM7180 is free software: you can redistriQThreadbute it and/or modify
...
bool EM7180::begin(uint8_t bus)
{
    ...
    if(bcm2835_init())
    {
        ...
        {
            cRead = readRegister(SentralStatus);
            if (cRead & 0x01)
            {
                if(cRead & 0x01) { }
                if(cRead & 0x02) { }
...
uint16_t EM7180::getRamVersion(void)
{
    ...
    nReturn = readRegister(RAMVersion1) << 8;
    nReturn |= readRegister(RAMVersion2);
    return nReturn;
}
...
uint16_t EM7180::getRomVersion(void)
{
    ...
    nReturn = readRegister(ROMVersion1) << 8;
    nReturn |= readRegister(ROMVersion2);
    return nReturn;
}
...
void EM7180::requestReset(void)
{
    ...
    {
        delay(10);
        if(!(readRegister(ResetRequest) & 0x01))
            break;
    }
}

void EM7180::setCalibParameter(uint8_t Sen_param[35][4])
{
    char param = 1;
    char STAT;
    int i;
    ...
    param = param | 0x80;
    writeRegister(LoadParamByte0, Sen_param[0][0]);
    writeRegister(LoadParamByte0, Sen_param[0][1]);
    writeRegister(LoadParamByte0, Sen_param[0][2]);
    writeRegister(LoadParamByte0, Sen_param[0][3]);
    ...
    {
      param = (i+1) | 0x80;
      writeRegister(LoadParamByte0, Sen_param[i][0]);
      writeRegister(LoadParamByte1, Sen_param[i][1]);
      writeRegister(LoadParamByte2, Sen_param[i][2]);
      writeRegister(LoadParamByte3, Sen_param[i][3]);
      writeRegister(ParamRequest, param);
    ...
    writeRegister(ParamRequest, 0x00);
}
...
void EM7180::setPassThroughMode()
{
    ...
    cAlCon = readRegister(0x54);
    // First put SENtral in standby mode
    writeRegister(0x54, cAlCon | 0x01);
    while(true)
    {
        if (readRegister(0x38) & 0x01)
            break;
        writeRegister(0x54, cAlCon | 0x01);
    }
    ...
    while(true)
    {
        if (readRegister(PassThruStatus) & 0x01)
            break;
        delay(5);
        writeRegister(PassThruControl, 0x01);
    }
...
void EM7180::setMasterMode()
{
    ...
    while(true)
    {
        if (!(readRegister(AlgorithmStatus) & 0x01))
            break;
        delay(5);
    }
}
...
char EM7180::readRegister(char subAddress)
{
    char cRet;
    char bufIn;
    ...
    do
    {
        cRet = bcm2835_i2c_write_read_rs(&subAddress, 1, &bufIn, 1);
        if(cRet != 0)
        {
            delay(100);
            bcm2835_i2c_end();
...
{  
    ...
    int i;
    uint8_t cRet;
    char bufOut[1] = {subAddress};

    cRet = bcm2835_i2c_write_read_rs(bufOut, 1, reinterpret_cast<char*>(dest), count);
    cRead = cRet;
}

void EM7180::M24512DFMreadBytes(uint8_t device_address, char data_address1, char data_address2, char count, char* dest)
{
    char bufOut[2];
    char cRet;

    bcm2835_i2c_setSlaveAddress(device_address);
    bufOut[0] = data_address1;
    bufOut[1] = data_address2;
    do
    {
        cRet = bcm2835_i2c_write_read_rs(bufOut, 2, dest, count);
        if(cRet != 0)
        {
            delay(100);
            bcm2835_i2c_end();
